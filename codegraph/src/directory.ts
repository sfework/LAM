...
/**
 * LAM fork extension: externalize the per-project index storage.
 *
 * When `CODEGRAPH_STORE_ROOT` is set, the index for a project is NOT written to
 * `<projectRoot>/.codegraph` (which pollutes the scanned repo and triggers the
 * Windows EBUSY watcher errors). Instead it lives under
 * `<CODEGRAPH_STORE_ROOT>/<basename>-<hash8>` — one isolated folder per project,
 * all collected under the caller's own data dir. The caller drives the library by
 * project path (`CodeGraph.open(projectRoot)`); it never needs to know the folder.
 *
 * Unset ⇒ original in-project `.codegraph` behaviour, byte-for-byte.
 */
function externalStoreDir(projectRoot: string): string | null {
  const base = process.env.CODEGRAPH_STORE_ROOT?.trim();
  if (!base) return null;
  const resolved = path.resolve(projectRoot).toLowerCase().replace(/[\\/]+$/, '');
  const hash = crypto.createHash('sha256').update(resolved).digest('hex').slice(0, 8);
  const base0 = path.basename(path.resolve(projectRoot)).replace(/[^A-Za-z0-9._-]/g, '_') || 'proj';
  return path.join(path.resolve(base), `${base0}-${hash}`);
}
...
export function getCodeGraphDir(projectRoot: string): string {
  return externalStoreDir(projectRoot) ?? path.join(projectRoot, codeGraphDirName());
}
